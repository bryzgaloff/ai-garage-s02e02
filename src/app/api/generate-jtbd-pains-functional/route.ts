...
- `;
    ...
    const pains = extractArrayFromText(painsContent);

    return NextResponse.json({ pains });
  } catch (error) {
    console.error("Functional pains generation error:", error);
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Failed to generate functional pains" },
      { status: 500 }
    );
  }
}